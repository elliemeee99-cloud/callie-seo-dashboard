...
CACHE_FILE = "seo_monthly_sales_v4.pkl"
...
def extract_table(df_raw, start_idx, end_idx):
    df = df_raw.iloc[start_idx:end_idx].copy()
    if df.empty: return pd.DataFrame()
    
    # 定位真实的表头行
    header_row_idx = 0
    for i in range(len(df)):
        if '总计' in [str(x).strip() for x in df.iloc[i].values if pd.notna(x)]:
            header_row_idx = i
            break
    ...
    if '总计' in df.columns:
        col_total = df['总计']
        if isinstance(col_total, pd.DataFrame): col_total = col_total.iloc[:, 0]
        df['Total'] = pd.to_numeric(col_total, errors='coerce').fillna(0)
    else:
        df['Total'] = 0.0
    ...
    if uploaded_file is not None:
        try:
            ...
            for i, row in df_raw.iterrows():
                ...
                if '总计' in row_strs: header_indices.append(i)
...
if 'monthly_data' in st.session_state:
    ...
    with st.container(border=True):
        ...
        for i, year in enumerate(sorted(df_yoy['Year'].unique())):
            ...
            fig1.add_trace(go.Scatter(
                ...
                hovertemplate=f'<b>{year}年%{{x}}月</b><br>非品牌词总计: $%%{{y:,.2f}}<extra></extra>'
            ))
    ...
    **📌 基于以上三大核心图表的业务洞察分析：**
    ...
    1. **年度同环比成长性（参考图 1）**：
       通过折现纵向对比，可直观判定业务在今年的**增长动能**。若今年折线（如 2026 年）整体浮于去年上方，且波峰未滞后，说明我们的非品牌词 SEO 获取了真实的同比增量突破，未受到行业大盘淡季的过度影响。
